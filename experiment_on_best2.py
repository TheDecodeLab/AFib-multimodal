...
config = pd.read_csv('best_configs.csv')
idf = int(argv[1])

com_arch = int(config.loc[idf,'com_arch'])
n_comp = int(config.loc[idf,'n_comp'])
num_heads = int(config.loc[idf,'num_heads'])
ecg_app = int(config.loc[idf,'ecg_app'])
aug_p = config.loc[idf,'aug_p']
n_inc = int(config.loc[idf,'n_inc'])
n_comp_ecg = int(config.loc[idf,'n_comp_ecg'])
n_comp_ehr = int(config.loc[idf,'n_comp_ehr'])
initial_learning_rate = config.loc[idf,'initial_learning_rate']
...
###################################################################################################
###################################################################################################
# ehr_data.to_csv('ready_EHR.csv',index=0)
# np.save('ready_ECG.npy',[ecg_data,ecg_data_denoise,ecg_data_denoise_filtered,fft_data,fft_fake,ecg_fake])
# y_dl.to_csv('ready_Y.csv',index=0)
...
[ecg_data,ecg_data_denoise,ecg_data_denoise_filtered,fft_data,fft_fake,ecg_fake] = np.load('ready_ECG.npy')
y_dl = pd.read_csv('ready_Y.csv')

ehr_data = ehr_data[bmi_filter].reset_index(drop=1)
...
y_dl = y_dl[bmi_filter].reset_index(drop=1)
# print(ehr_data.shape,y_dl.shape,y_dl.value_counts())
# exit()
df_res = pd.DataFrame(columns=['AUROC','Precision','Sensitivity','Specificity','Accuracy','F1 score'])
df_pred = pd.DataFrame(index=y_dl.index,columns=['y_true','y_pred'])
...
for _ in range(n_try):
    kf = KFold(n_splits=5, shuffle=True)
    kf.get_n_splits(np.arange(n_data))
    for i, (train_index, test_index) in enumerate(kf.split(y_dl)):
        pbar.update(1)
        ehr_data_train = ehr_data_.iloc[train_index]
        ehr_data_test = ehr_data_.iloc[test_index]
        ...
        ecg_data_train = ecg_data_[train_index]
        ecg_data_test = ecg_data_[test_index]
        y_train = y_dl.loc[train_index].values[:,0]
        y_test = y_dl.loc[test_index].values[:,0]
        assert np.mean(y_train)*np.mean(y_test)!=0, 'Data problem 1'
        ...
        model = model_maker(ecg_shape, ehr_shape, n_comp_ecg=n_comp_ecg, n_comp_ehr=n_comp_ehr)
        print(ecg_data_train.shape,ehr_data_train.shape,y_train.shape)
        # exit()
        ...
        df_pred.loc[test_index,'y_true'] = y_test
        df_pred.loc[test_index,'y_pred'] = y_pred_aug.reshape(n_inc, -1).mean(axis=0)
...
np.save(f'res_best2/{fname}',info)
df_res.to_csv(f'res_best2/{fname}.csv')
df_pred.to_csv(f'res_best2/{fname}_pred.csv')
